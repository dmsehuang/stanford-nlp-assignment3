...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            b_r = tf.get_variable("b_r", shape=[self._state_size], initializer=tf.constant_initializer())
            ...
            b_z = tf.get_variable("b_z", shape=[self._state_size], initializer=tf.constant_initializer())
            ...
            b_o = tf.get_variable("b_o", shape=[self._state_size], initializer=tf.constant_initializer())

            z_t = tf.sigmoid(tf.matmul(inputs, W_r) + tf.matmul(state, U_r) + b_r)
            r_t = tf.sigmoid(tf.matmul(inputs, W_z) + tf.matmul(state, U_z) + b_z)
