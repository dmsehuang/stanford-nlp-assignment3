    ...
    for sentence, labels in data:
        ...
        for i, label in enumerate(labels):
            ...
            for j in range(i-window_size, i+window_size+1):
                w = start if j < 0 else end if j >= len(labels) else sentence[j]
                vec.extend(w)
            windowed_data.append((vec, label))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_loss_op(self, pred):
        ...
        res = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,logits=pred)
